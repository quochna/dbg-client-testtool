 ...
 * @author hainpt
 */
public class SmsSendMOController extends DbgClientCore
{
    private static Logger logger = Logger.getLogger(SmsSendMOController.class);
    private final String ITEM_SEPARATE = "\\|";
    private final String PARAM_STATS = "stats";
    private final Monitor readStats = new Monitor();

    public SmsSendMOController()
    {
    }
    ...
    private String renderByTemplate(HttpServletRequest request) throws TemplateException, ParseException
    {
        ...
        dic.showSection("smsSendMO");
        //yyMMddhhmmssSSS
        SimpleDateFormat sdf = new SimpleDateFormat("yyMMddHHmmssSSS");
        SimpleDateFormat sdf1 = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
        Date curDate = DateTimeUtil.getCurDateWithMilisec();
        
        String requestID = "21" + sdf.format(curDate);
        String message = request.getParameter("momessage");
        
        dic.setVariable("requestid", requestID);
        dic.setVariable("userid", "84902078736");
        dic.setVariable("serviceid", "6069");
        dic.setVariable("commandcode", "PAY1");
        dic.setVariable("message", message);
        dic.setVariable("mobileoperator", "VMS");        
        dic.setVariable("username", "test");
        dic.setVariable("password", "test");
        dic.setVariable("requesttime", sdf1.format(curDate));       
